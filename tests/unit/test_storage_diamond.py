...
def test_invalidate_downstream_from_a_marks_diamond_descendants_stale_once(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "test_1.db"

    with SQLiteStorage(str(db_path)) as storage:
        _insert_success_diamond(storage)
...
def test_invalidate_downstream_from_b_marks_only_d_stale_in_diamond(
    tmp_path: Path,
) -> None:
    ...
    with SQLiteStorage(str(db_path)) as storage:
        _insert_success_diamond(storage)
        storage.invalidate_downstream("B")
...
def test_invalidate_downstream_from_d_keeps_all_diamond_nodes_success(
    tmp_path: Path,
) -> None:
    ...
    with SQLiteStorage(str(db_path)) as storage:
        _insert_success_diamond(storage)
        storage.invalidate_downstream("D")

    assert _node_statuses_from_sql(db_path) == {
        "A": NodeStatus.SUCCESS.value,
        "B": NodeStatus.SUCCESS.value,
        "C": NodeStatus.SUCCESS.value,
        "D": NodeStatus.SUCCESS.value,
    }


def _insert_success_diamond(storage: SQLiteStorage) -> None:
    for node_id in NODE_IDS:
        storage.save_node(_make_success_node(node_id))

    for parent_node_id, child_node_id in DIAMOND_EDGES:
